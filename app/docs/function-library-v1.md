...
| `str.slice(s, start, len?)` | substring; out-of-range clamps, never errors |
...
| `list.map(l, fnAttr, d?)` | evaluate an attribute-held function per element — **creates a frame per element** (recursion-charged) and each element evaluation draws on the same fuel meter |
| `list.filter(l, fnAttr, d?)` | same charging model as `map` |
...
| `obj.resolve(name)` | name matching per GM-R12: `me`, `here`, partial names, `#dbref` — match work charges fuel |
...
| `ctl.iter(l, body, d?)` | bounded iteration over a list; per-element frame + fuel charges — unbounded iteration is impossible by construction because iteration count ≤ list element count and every element charges |
...
| `out.style(text, spec)` | wrap text in GenMURK's markup tokens (bold/color classes per GM-R13); the *renderer* maps tokens to ANSI/markup at the transport — softcode never emits raw escape bytes, so styled output cannot smuggle terminal control sequences |
